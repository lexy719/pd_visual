...
export function hardenImages(sections: SectionResult[]): number {
  let changed = 0
  for (const s of sections) {
    s.code = s.code.replace(/<img\b([^>]*?)(\/?)>/gs, (full, attrs: string, selfClose: string) => {
      ...
      if (!add) return full
      changed++
      return `<img${attrs}${add}${selfClose ? ' /' : ''}>`
    })
  }
  return changed
}
...
async function genScratch(
  ...
  shot?: { beat: ShotBeat; dominant: boolean },
...
export async function generateSections(
  ...
  for (let i = 0; i < plan.sections.length; i++) {
    ...
    if (prim) {
      ...
      let code = await genUse(plan, section, comp, importPath, mi, 'bulk')
      ...
      if (!usesComponent(code, importPath)) {
        ...
        tier = 'bulk→escalated'
        code = await genUse(plan, section, comp, importPath, mi, 'reasoning', true)
...

    {
      ...
      // This section's slice of the locked shot plan (non-photo moods get no beat — no photos there).
